...
#include "../../core/fdaPDE/linear_algebra/rand_svd.h" // to be removed
...
template <> class RegularizedSVD<sequential> {
  ...
  int max_iter_ = 20;         // maximum number of allowed iterations
    ...
    template <typename ModelType> struct rsvd_iterator {
      ...
      // thin monolithic Singular Value Decomposition (not regularized)
      // we do not store svd because uses a lot of memory, store just the results
      /*Eigen::JacobiSVD<DMatrix<double>>*/ /*fdapde::core::RSI<DMatrix<double>, core::adaptive>*/
      DMatrix<double> V_;   
        ModelType& model_;
        ...
        void rank_one_step() {
	  DVector<double> f0 = /*svd_.matrixV().*/V_.col(index_);
	  ...
	  Eigen::JacobiSVD<DMatrix<double>> svd_(X_, Eigen::ComputeThinU | Eigen::ComputeThinV);
	  // fdapde::core::RSI<DMatrix<double>, core::adaptive> svd(X_, 3, 2, 1e-4);
	  // V_ = read_csv<double>("../../../../università/articoli/neurocose/svd_matrixV.csv").leftCols(3);// svd.matrixV();
	  V_ = svd_.matrixV();
	  std::cout << V_.rows() << "x" << V_.cols() << std::endl;
